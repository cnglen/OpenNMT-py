...
class DecodeStrategy(object):
    ...
    Attributes:
        ...
        attention (list[list[FloatTensor or list[]]]): For each batch, holds a list of attention sequence tensors (or empty lists) having shape ``(step, inp_seq_len)`` where
            ``inp_seq_len`` is the length of the sample (not the max length of all inp seqs).
    ...
    def __init__(self, pad, bos, eos, batch_size, device, parallel_paths,
        ...
        # result caching: length of batch_size
        self.predictions = [[] for _ in range(batch_size)]
        self.scores = [[] for _ in range(batch_size)]
        self.attention = [[] for _ in range(batch_size)]

        self.alive_seq = torch.full([batch_size * parallel_paths, 1], self.bos, dtype=torch.long, device=device)  # (batch_size*parallel_paths, T=1)
        self.is_finished = torch.zeros([batch_size, parallel_paths], dtype=torch.uint8, device=device)  # (batch_size, parallel_paths)
    ...
    def ensure_max_length(self):
        ...
        强制is_finished为1
        """
        # add one to account for BOS. Don't account for EOS because hitting this implies it hasn't been found.
        if len(self) == self.max_length + 1:
            self.is_finished.fill_(1)

    def block_ngram_repeats(self, log_probs):
        """
        强制将包含n-gram的path分数设为很低
    ...
    def update_finished(self):
        ...
        ``update_finished`` is used to update
        - ``self.predictions``,
        - ``self.scores``,
