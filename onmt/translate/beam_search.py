...
class BeamSearch(DecodeStrategy):
    ...
    Attributes:
        ...
        alive_seq (LongTensor): See base. topk_log_probs (FloatTensor): Shape ``(B x beam_size,)``. These are the scores used for the topk operation.
    ...
    def __init__(self, beam_size, batch_size, pad, bos, eos, n_best, mb_device,
        ...
        super(BeamSearch, self).__init__(pad, bos, eos, batch_size, mb_device, beam_size, min_length, block_ngram_repeat, exclusion_tokens, return_attention, max_length)
        # beam parameters
        ...
        self.topk_scores = torch.empty((batch_size, beam_size), dtype=torch.float, device=mb_device)
        self.topk_ids = torch.empty((batch_size, beam_size), dtype=torch.long, device=mb_device)
        self._batch_index = torch.empty([batch_size, beam_size], dtype=torch.long, device=mb_device)
        self.done = False
    ...
    def advance(self, log_probs, attn):
        """
        更新self.alive_seq, self.is_finished, self.alive_attn
        ...
        if self._stepwise_cov_pen and self._prev_penalty is not None:
            ...
            self.topk_log_probs -= self.global_scorer.cov_penalty(self._coverage + attn, self.global_scorer.beta).view(_B, self.beam_size)
        ...
        if self.return_attention or self._cov_pen:
            ...
            if step == 1:
                ...
                if self._cov_pen:
                    ...
                    self._coverage += current_attn
                    self._prev_penalty = self.global_scorer.cov_penalty(self._coverage, beta=self.global_scorer.beta).view(_B, self.beam_size)
    ...
    def update_finished(self):
        ...
        attention = (self.alive_attn.view(step - 1, _B_old, self.beam_size, self.alive_attn.size(-1)) if self.alive_attn is not None else None)
        non_finished_batch = []
            ...
            for j in finished_hyp:
                ...
                self.hypotheses[b].append(
                    (
                        ...
                        attention[:, i, j, :self._memory_lengths[i]] if attention is not None else None
                    )
                )
